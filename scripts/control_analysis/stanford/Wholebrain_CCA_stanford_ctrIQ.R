...
# regressing out IQ (fsiq) from math and reading measures.
# PCA is used on GMV data for dimensionality reduction.
#
# Key differences between tasks:
#  1) Math uses residuals of "numop_std" and "mathrea_std"
#     (after regressing out IQ).
...
# Load utility functions: load_data, identify_outliers, mypca, mycca_noscale, etc.
...
analyses <- list(
  math = list(
    output_path = "results/cca/stanford/wholebrain_cca_stanford_math_ctrIQ",
    postfix     = "mathstd_ageinmodel",
    cols        = c("age", "numop_std", "mathrea_std", "fsiq")
  ),
  reading = list(
    output_path = "results/cca/stanford/wholebrain_cca_stanford_reading_ctrIQ",
    postfix     = "readstd_ageinmodel",
    cols        = c("age", "wordread_std", "readcomp_std", "fsiq")
  )
)
...
for (task in names(analyses)) {
  cat("\n----------------------------\n")
  cat("Running CCA for Stanford:", task, "\n")
  cat("----------------------------\n")
  ...
  # ----------------------------------------
  if (task == "math") {
    numop_resid   <- resid(lm(numop_std ~ fsiq, data = YY))
    mathrea_resid <- resid(lm(mathrea_std ~ fsiq, data = YY))
    Y <- data.frame(
      age = YY$age,
      numop_std_resid = numop_resid,
      mathrea_std_resid = mathrea_resid
    )
  } else if (task == "reading") {
    wordread_resid <- resid(lm(wordread_std ~ fsiq, data = YY))
    readcomp_resid <- resid(lm(readcomp_std ~ fsiq, data = YY))
    Y <- data.frame(
      age = YY$age,
...
cat("\nAll analyses (Reading and Math) with IQ control for Stanford completed!\n")
