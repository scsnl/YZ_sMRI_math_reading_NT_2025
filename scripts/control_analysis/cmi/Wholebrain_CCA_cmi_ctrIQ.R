...
# regressing out IQ (WISC_FSIQ) from math and reading 
# measures before CCA. PCA is used on GMV data for 
# dimensionality reduction.
#
# Key differences between tasks:
#  1) Math uses residuals of "numop_std" and "mathprob_std"
#     (after regressing out WISC_FSIQ).
...
analyses <- list(
  math = list(
    output_path = "results/cca/cmi/wholebrain_cca_cmi_math_ctrIQ",
    postfix     = "mathstd_ageinmodel",
    cols        = c("age", "numop_std", "mathprob_std", "WISC_FSIQ")
  ),
  reading = list(
    output_path = "results/cca/cmi/wholebrain_cca_cmi_reading_ctrIQ",
    postfix     = "readstd_ageinmodel",
    cols        = c("age", "wordread_std", "readcomp_std", "WISC_FSIQ")
  )
)
...
for (task in names(analyses)) {
  ...
  # ----------------------------------------
  # Regress out IQ (WISC_FSIQ) from measures
  # ----------------------------------------
  if (task == "math") {
    numop_resid    <- resid(lm(numop_std ~ WISC_FSIQ, data = YY))
    mathprob_resid <- resid(lm(mathprob_std ~ WISC_FSIQ, data = YY))
    Y <- data.frame(
      age = YY$age,
    ...
    wordread_resid <- resid(lm(wordread_std ~ WISC_FSIQ, data = YY))
    readcomp_resid <- resid(lm(readcomp_std ~ WISC_FSIQ, data = YY))
    Y <- data.frame(
      age = YY$age,
...
cat("\nAll analyses (Reading and Math) with IQ control completed!\n")
