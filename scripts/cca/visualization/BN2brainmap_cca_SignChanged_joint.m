...
%   3) Save whole-brain GMV weight maps (NIfTI, float32, compressed .nii.gz).
%   4) Save ROI-level source data used to generate the NIfTI maps.
%   5) Save thresholded top-percentile GMV weight maps and their ROI indices.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
postfix = "shared_SignChanged_cca";
...
configs = {
    % CMI
    'cmi', 'wholebrain_cca_cmi_math_reading_combined/CCA_PCA_roi_gmv_brainnetome_mathreadingstd_ageinmodel_coef.csv', 'cmi_mathread';

    % Stanford
    'stanford', 'wholebrain_cca_stanford_math_reading_combined/CCA_PCA_roi_gmv_brainnetome_mathreadingstd_ageinmodel_coef.csv', 'stanford_mathread';
};
...
for c = 1:size(configs, 1)
    ...
    for ii = 1:length(col_idx)
        ...
        % ------------------------------------------------------------------
        % 4.1 Save ROI-level source data for whole-brain NIfTI map
        % ------------------------------------------------------------------
        roi_id = (1:nroi)';
        ...
        source_table = table( ...
            ...
            dat(:), ...
            dat_sign_oriented(:), ...
            'VariableNames', {'ROI_ID', 'Original_xloading_m2', 'Sign_oriented_value_used_for_NIfTI'} ...
        );
